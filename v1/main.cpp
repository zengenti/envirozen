...
//// Calculate the free memory of the board.
int freeMemory() {
...
void metricsCmd(Request &req, Response &res) // Prom Metrics
{
  ...
  res.print("# HELP free_memory is the amount of free memory in bytes\n");
  res.print("# TYPE free_memory gauge\n");
  res.print("free_memory " + String(freeMemory()) + "\n");
}
...
void controlCmd(Request &req, Response &res) 
{
  ...
  res.println("<html>");
  res.println("<head>");
  res.println("<meta http-equiv='refresh' content='30; url=/control'>");
...
{
  Serial.println("Request for Auto");
  res.set("Content-Type", "text/html");
...
{
  Serial.println("Request for Fans");
  res.set("Content-Type", "text/html");
